...
public class New2 extends HttpServlet 
{
	...
	try 
		...
		if(rs.next()) 
		    ...
		    {
			String query2 = "insert into users (login,mdp,email,role,tel) values (";
			query2 += ("'"+login+"',");
			query2 += ("'"+mdp+"',");
			query2 += ("'"+email+"',");
			query2 += ("'usr',");
			...
			out.println("<div class='alert alert-success' role='alert'>Bienvenue " + login + ", ! Votre compte à bien été créé !</div>");
